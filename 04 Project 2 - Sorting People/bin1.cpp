...
void sortDataList(list<Data *> &l) {
   // Data arr[30] = {};
    ...
    for (auto &it : lastMap) {
        ...
        for (int i = 0; i <= k; ++i) {
            if (i != k && pointer[i]->firstName == pointer[i + 1]->firstName) {
                vector<Data *> xst = {};
                int z = i;
                xst.push_back(pointer[i]); //this one
                ++i;
                xst.push_back(pointer[i]); //the next one
                while(i < k && pointer[i + 1]->firstName == pointer[z]->firstName) {
                    ++i;
                    xst.push_back(pointer[i]); //this one
                }
                sort(xst.begin(),xst.end(),comparator);
                for (auto &rep : xst){
                    *listIt = rep;
                    ++listIt;
                }
    ...
    // insert into the x list and sort at the same time - either a binary tree or just start and end and work our way back
    // do merge on the two lists
